...
void draw() {
  ...
  //resoPosF(resoNorm(int(user1.x*width), width), resoNorm(int(height-(user1.y*height)), height));
...
void resoPos(int X, int Y){
  ...
  OscMessage myMessage = new OscMessage("/composition/layers/4/clips/1/video/effects/transform/positionx");
  myMessage.add(resoNorm(X, 1280));
  myBundle.add(myMessage);
  myMessage.clear();
  myMessage.setAddrPattern("/composition/layers/4/clips/1/video/effects/transform/positiony");
  myMessage.add(resoNorm(Y, 480));
  myBundle.add(myMessage);
  oscP5.send(myBundle, resoOSC);
}

void resoPosF(float X, float Y){
  OscBundle myBundle = new OscBundle();
  OscMessage myMessage = new OscMessage("/composition/layers/4/clips/1/video/effects/transform/positionx");
  myMessage.add(X);
  myBundle.add(myMessage);
